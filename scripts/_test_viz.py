...
check("meta вне строки покрытия отклоняется", any("Показано" in r for r in bld('<div class="viz"><p>жалоб {{meta:facts_total}} тыс. {{f:2}} {{f:2.cite}}</p></div>')))
check("строка покрытия в div без small проходит", not bld('<div class="viz"><p>{{f:1}} {{f:1.cite}}</p><div>Показано фактов: {{meta:facts_used}} из {{meta:facts_total}}; объектов {{meta:subjects}}</div></div>', section="voice"))
check("цитата с якорем в подписи следующей строки проходит", not bld('<div class="viz"><p>«{{f:3.quote}}»</p><small>{{name:sberbank}} {{f:3.cite}}</small></div>', section="voice"))
check("число с якорем в соседнем элементе — по-прежнему отказ", any("том же элементе" in r for r in bld('<div class="viz"><p>{{f:3}}</p><small>{{f:3.cite}}</small></div>', section="voice")))
...
print("\n— после живого прогона 7 —")
check("единица не дублируется: «1%» + % → «1%»", V.value_with_unit(fact(1, "s", "a", "1%", "%")) == "1%" and V.value_with_unit(fact(1, "s", "a", "300 000 ₽", "₽")) == "300 000 ₽")
check("синоним единицы: «60 млн рублей» + ₽", V.value_with_unit(fact(1, "s", "a", "более 60 млн рублей", "₽")) == "более 60 млн рублей")
check("единица добавляется, когда её нет", V.value_with_unit(fact(1, "s", "a", "от 1,25", "%")) == "от 1,25 %" and V.value_with_unit(fact(1, "s", "a", "2", "дня")) == "2 дня")
check("простыня обрезается", len(V._clip("x " * 200)) <= V.VALUE_CHARS + 1 and V._clip("x " * 200).endswith("…"))
_fin = V.finalize('<div class="viz"><small>заявлено · </small><p>' + V._S_CITE + '1' + V._S_CITE + '</p></div>', {}, cite=lambda fid: 2)
check("висячий разделитель после пустой даты убран", "заявлено</small>" in _fin)
check("длинное значение помечено в списке фактов", "длинное значение" in V.mark_long_values("[f:1] Сбер | Стоимость | " + "1 900 руб. " * 20 + " | заявлено") and "длинное" not in V.mark_long_values("[f:1] Сбер | Ставка | 1% | заявлено"))
