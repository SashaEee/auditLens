    ...
    while changed:
        ...
        for p in _LEGAL_FORM_PREFIXES:
            if s.startswith(p):
                s = s[len(p):].strip()
                changed = True
    # Слово «банк» в начале или в конце — не часть имени, а его оформление:
    # источники пишут вперемешку «ВБРР» и «Банк ВБРР», «АГОРА» и «Банк «АГОРА»»,
    # «Агророс Банк» и «АГРОРОС». Каждое написание порождало отдельную запись в
    # справочнике, и один банк считался двумя точками рынка (аудит 11.08.2026).
    # Одиночное «банк» не трогаем — иначе имя схлопнется в пустую строку.
    for _ in range(2):
        if s.startswith("банк ") and len(s) > 5:
            s = s[5:].strip()
        if s.endswith(" банк") and len(s) > 5:
            s = s[:-5].strip()
    return s
