...
_CAVEAT_RE = re.compile(r"\bпри\b|\bесли\b|в\s+случае|услови|"
                        ...
                        r"иначе|подписк\w*\s+(?:на|обязат)|за\s+первый|"
                        # «бесплатно ДЛЯ НОВЫХ клиентов» — тоже условие, просто
                        # выраженное не союзом, а адресатом
                        r"для\s+(?:нов|зарплатн|клиент|держател\w+\s+пакет)", re.I)


def _free_by_rule(text: str, fee: float | None) -> str | None:
    """«unconditional», если так говорит подпись тарифа и не спорит цена.

    Цена может быть НЕ разобрана: у кредиток карточка каталога вообще не несёт
    поля «Обслуживание» (там грейс и ПСК), и требовать совпадения двух сигналов
    значило бы отказаться от разбора для всей категории — 42 карты из 53 висели
    «неизвестно». Подпись на детальной странице — утверждение самого источника,
    и опираться на неё честно. А вот НЕНУЛЕВАЯ цена правило запрещает всегда.
    """
