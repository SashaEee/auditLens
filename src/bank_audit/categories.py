...
# ── Карта покрытия ──────────────────────────────────────────────────────────
# Чего в витрине нет и ПОЧЕМУ. Раньше здесь была одна строка про металлы, и она
# никуда не выводилась: аудитор искал драгметаллы, страхование, инвалюту, не
# находил и не мог понять — то ли этого нет на рынке, то ли инструмент этого не
# собирает. Четыре замечания пользователей — ровно об этом. Молчание
# хуже отказа: «не собираем» должно быть видно так же ясно, как сами данные.
#
#   reason — человеческая причина, её читает аудитор на витрине;
#   status — что требуется, чтобы закрыть: 'источник не подключён' (нужен новый
#     сборщик), 'нет сопоставимой метрики' (сравнивать нечем по природе
#     продукта), 'собирается, но не сравнивается' (данные есть, ранга нет).
NOT_COVERED: dict[str, dict] = {
    "metals": {
        "label": "Драгметаллы и ОМС",
        "reason": "Раздела со ставками по обезличенным металлическим счетам у "
                  "агрегаторов нет, а доход по металлам курсовой: сравнивать "
                  "банки по «ставке» здесь нечем.",
        "status": "источник не подключён",
        "needs": "котировки ЦБ + тарифы банков (два потока)"},
    "insurance": {
        "label": "Страховые продукты",
        "reason": "Тарифы страховщиков отдельным источником не собираются. "
                  "Что уже видно: страховка как условие ставки — по офферам "
                  "рынка видно, у скольких минимальная ставка куплена полисом.",
        "status": "источник не подключён",
        "needs": "адаптер витрины страховых продуктов"},
    "osago": {"label": "ОСАГО", "reason": "Тарифы ОСАГО отдельным источником не собираются.",
              "status": "источник не подключён", "needs": "адаптер страховой витрины"},
    "kasko": {"label": "КАСКО", "reason": "Тарифы КАСКО отдельным источником не собираются.",
              "status": "источник не подключён", "needs": "адаптер страховой витрины"},
    "insurance_life": {"label": "Страхование жизни (ИСЖ/НСЖ)",
                       "reason": "Отдельным источником не собирается.",
                       "status": "источник не подключён", "needs": "адаптер страховой витрины"},
    "insurance_mortgage": {"label": "Страхование ипотеки",
                           "reason": "Отдельным источником не собирается.",
                           "status": "источник не подключён", "needs": "адаптер страховой витрины"},
    "insurance_travel": {"label": "Страхование путешествий",
                         "reason": "Отдельным источником не собирается.",
                         "status": "источник не подключён", "needs": "адаптер страховой витрины"},
    "currency_exchange": {
        "label": "Продукты в иностранной валюте",
        "reason": "Витрина сравнивает только рублёвые предложения: валютная "
                  "ставка не сопоставима с рублёвой напрямую, а курс — "
                  "самостоятельное измерение.",
        "status": "нет сопоставимой метрики",
        "needs": "валюта как отдельный разрез витрины"},
    "invest_pif": {
        "label": "ПИФы",
        "reason": "Доходность фондов не сравнима со ставкой и меняется задним "
                  "числом; отдельный источник не подключён.",
        "status": "источник не подключён",
        "needs": "адаптер витрины фондов"},
    "invest_broker": {
        "label": "Брокеры",
        "reason": "Организации собираются как справочник (кто есть на рынке), "
                  "но не ранжируются: у брокера нет ставки, сопоставимой между "
                  "игроками — есть оценка организации.",
        "status": "собирается, но не сравнивается",
        "needs": "справочная полка вместо ранга"},
    "npf": {
        "label": "Негосударственные пенсионные фонды",
        "reason": "Собираются как справочник; доходность НПФ раскрывается "
                  "по итогам года и не сопоставима со ставкой.",
        "status": "собирается, но не сравнивается",
        "needs": "справочная полка вместо ранга"},
    "leasing": {"label": "Лизинг", "reason": "Источник не подключён.",
                "status": "источник не подключён", "needs": "адаптер"},
    "factoring": {"label": "Факторинг", "reason": "Источник не подключён.",
                  "status": "источник не подключён", "needs": "адаптер"},
    "acquiring": {"label": "Эквайринг", "reason": "Источник не подключён.",
                  "status": "источник не подключён", "needs": "адаптер"},
    "business_loan": {"label": "Кредиты бизнесу", "reason": "Источник не подключён.",
                      "status": "источник не подключён", "needs": "адаптер"},
    "refinance": {"label": "Рефинансирование", "reason": "Источник не подключён.",
                  "status": "источник не подключён", "needs": "адаптер"},
    "mortgage_refinance": {"label": "Рефинансирование ипотеки",
                           "reason": "Источник не подключён.",
                           "status": "источник не подключён", "needs": "адаптер"},
}

# Обратная совместимость: прежнее имя с одной строкой.
RETIRED = {k: v["reason"] for k, v in NOT_COVERED.items()}


def coverage_note(category: str) -> dict | None:
    """Почему категории нет в витрине. None — категория покрыта."""
    return NOT_COVERED.get(category)
