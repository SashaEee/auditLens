...
def report_prompt(plan, question: str, *, has_ranking: bool = False,
    ...
    parts = [
        ...
        "4. У фактов две стороны. «заявлено» — со слов самой организации; "
        ...
        "5. Где по объекту факта нет — так и напиши, коротко и без домыслов.",
        "6. Заголовки — по-русски, без английских слов и служебных обозначений "
        "в скобках. Раздел со списком источников НЕ пиши: он собирается "
        "автоматически по якорям.",
        "7. Не пиши, что источники тебе «предоставлены» или «даны в подборке»: "
        "они найдены в ходе исследования.",
        "8. Где объекты сопоставимы — приводи таблицу сравнения.",
    ]
    if has_regulatory:
        parts += [
            "9. Обязателен раздел «Нормативная рамка»: какие требования "
    ...
    if has_ranking:
        parts += [
            "10. Обязателен раздел «Ранжирование». В контексте дана посчитанная "
            "полнота раскрытия — приведи её как есть и ЯВНО назови критерий "
            "(«ранжируем по полноте раскрытия, а не по выгодности продукта»). "
            "Если по существу вопроса уместен другой порядок — приведи и его, "
            "но тогда критерий назови отдельно и обоснуй фактами.",
        ]
    parts += [
        "",
