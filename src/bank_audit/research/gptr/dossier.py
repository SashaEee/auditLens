...
_SECTION_RULES = {
    ...
    "market": (
        ...
        "ОСНОВА СРАВНЕНИЯ — условия из источников самих банков (тарифы, "
        "страницы продуктов, документы): ставка вместе с условием её получения, "
        "срок, сумма, комиссии, требования, ограничения, шаги процесса. По КАЖДОМУ "
        "банку вопроса — столько подробностей, сколько есть в фактах; банк, по "
        "которому фактов мало, — так и отметь в таблице, не пропускай молча. "
        "Факты «витрина «Рынок» AuditLens» — сверочные: место на рынке и контроль "
        "чисел; подробностей условий в них нет, и заменять ими условия банков "
        "нельзя. Если витрина расходится с источником банка — покажи оба и "
        "объясни (другой продукт, дата, акция). Ставку приписывай только тому "
        "банку, у которого она в факте. Промо-условия (короткий срок, только "
        "новые деньги, требования к пакету) оговаривай рядом с числом: лучшая "
        "ставка на 3 месяца и базовая на год — "        "несопоставимы."
    ),
    "voice": (
...
async def write_dossier(client, model: str, *, question: str, plan, registry,
    ...
    # Вопрос сравнивает несколько банков — раздел сравнения обязателен и первым:
    # замер 26.09, «сравни путь оформления карты у 4 банков» — бриф выбросил его,
