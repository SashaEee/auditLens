...
def _common(plan, question: str, labels: dict[str, str]) -> str:
    ...
    return "\n".join([
        ...
        f"СЕГОДНЯ {today}. ДЕЙСТВУЕТ ИЛИ БУДЕТ: условие, норма или ставка с датой "
        f"вступления в силу позже сегодняшней — будущее правило; называй его с датой "
        f"(«с 1 октября — 12%») и не выдавай за действующее. Материал о грядущих "
        f"изменениях («новые ставки», «изменили условия с …») описывает будущее, "
        f"пока дата не наступила.",
...
_SECTION_RULES = {
    ...
    "market": (
        ...
        "«ОТ X» — ГРАНИЦА, А НЕ СТАВКА. «от 1,3%» у одного банка и «1,7% при "
        "обороте от 500 тыс.» у другого несопоставимы: первое — лучший возможный "
        "случай, второе — ставка для заданного условия. Сравнивай «от» с «от», а "
        "по условию вопроса (оборот, сумма, срок) — только ставки для этого "
        "условия; где у банка есть только «от», так и напиши в таблице и не "
        "ставь его выше или ниже других.\n"
    ...
    "voice": (
        ...
        "3. Конкуренты — тем же образом, короче, если по ним есть жалобы. Число "
        "жалоб зависит от размера бизнеса: у крупного банка их больше просто "
        "потому, что больше клиентов. Сравнивая банки по абсолютному числу, "
        "оговаривай это и опирайся на то, что от размера не зависит: структуру "
        "тем (доли), динамику к прошлому периоду, долю эскалации.\n"
...
def section_prompt(section: str, plan, question: str, labels: dict[str, str],
    ...
    if section == "checks":
        parts += ["", _CHECKS_LOOPHOLES if "loopholes" in (order or ()) else _CHECKS_NO_LOOPHOLES]
    if section in LEAD:
        if prior_text:
...
async def write_dossier(client, model: str, *, question: str, plan, registry,
    ...
    # Лазейки — раздел, только если бриф счёл схемы относящимися к вопросу:
    # принудительный раздел в эквайринге и пути оформления карты превращался
