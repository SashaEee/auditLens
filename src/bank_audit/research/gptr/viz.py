...
_FORMS = {
    "conditions": (
        "Матрица «объект × характеристика»: свой банк первой строкой, "
    ...
    "summary": (
        "Карточка ГЛАВНОГО ОТВЕТА — зеркало текста резюме, своего вывода не "
        "придумывай. Состав: (1) вопрос проверки одной строкой словами; (2) "
        "главный вывод — одна-две фразы, как в первом абзаце резюме (числа — "
        "только плейсхолдерами); (3) «Ключевые цифры» — три-шесть строк "
        "«показатель — значение» или компактная таблица «банк × показатель» "
        "из тех чисел, на которых стоит вывод резюме; у каждого значения видно, "
        "что это ({{f:N.attr}} или заголовок столбца) и единица — число жалоб и "
        "ставка в процентах не могут стоять в одной строке как одно и то же; "
        "(4) первые шаги проверки — <ol>, как в резюме. Пары «заявлено против "
        "наблюдается» — только если они есть в тексте резюме. Строк и ячеек "
        "«нет данных» в этой карточке не делай: чего нет — того в ней нет. В "
        "таблице «банк × показатель» у каждого значения — {{f:N.date}} рядом "
        "(или одна общая дата в заголовке, если она у всех одна). Якорь — только "
        "{{f:N.cite}}; запись «[f:N]» в тексте карточки — нарушение."),
}
...
def designer_prompt(*, section: str, title: str, question: str, anchor: str,
    ...
    # «точка отсчёта» — термин промпта, в карточке читателю он непонятен
    ...
    return "\n".join([
        ...
        "упоминай: легенда пишется словами («рамкой выделен свой банк»). "
