...
СТРАТЕГИЯ (web-first, БД может быть неполной):
  ...
  2. web_search свежих отзывов — СЕЛЕКТИВНО, 1-2 ТОЧЕЧНЫХ запроса на банк, НЕ
     перебирай 4-5 доменов подряд. Лучший один: site:banki.ru/services/responses
     ("{банк} {тема} отзыв жалоба"); при нужде второй — site:irecommend.ru или
     site:otzovik.com. После 1-2 поисков уже есть кандидаты — переходи к чтению.
  3. Из найденного ОТБЕРИ по title+snippet 2-4 самые информативные страницы
     отзывов и прочитай ТОЛЬКО их (read_url). Нерелевантное (не про тему/не про
     этот банк) — НЕ читай. Извлекай конкретные жалобы с дословными цитатами и датами.
