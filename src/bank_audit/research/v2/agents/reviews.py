...
СТРАТЕГИЯ (DB-first — основной источник жалоб уже готов):
  1. search_reviews_db — ПЕРВЫМ И ОСНОВНЫМ (корпус banki.ru: ~390 тыс. жалоб
     ...
     ⚠⚠ Инструмент по умолчанию ищет ТОЧЕЧНО по каждому из твоих банков (# ОБЪЕКТЫ)
     и возвращает by_bank:{банк:[жалобы]} — НЕ нужно перечислять банки вручную и НЕ
     надо брать произвольные. Для одного банка можно явно bank=<банк>; banks=[...] —
     лишь если надо сузить набор. НИКОГДА не пиши «у банка X жалоб нет», если X есть в
     # ОБЪЕКТЫ, но ты не искал по нему (точечный поиск по нему обязателен).
