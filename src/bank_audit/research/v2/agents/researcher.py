...
СТРАТЕГИЯ — СНАЧАЛА СТРУКТУРА И КЭШ, ПОТОМ ВЕБ (меньше шума И меньше капчи):
  ...
  2. Только то, чего НЕ хватает, добирай вебом: до %MS% ТОЧЕЧНЫХ web_search на
     банк (site:{bank.ru} + тема, и один независимый site:banki.ru). НЕ гони поиск
     бесконечно — после первых у тебя уже есть кандидаты.
  3. Из найденного прочитай read_url до %MR% САМЫХ релевантных. ПРИОРИТЕТ ИСТОЧНИКА:
...
# Лимиты сбора — env-тюнингом (V2_MAX_SEARCHES / V2_MAX_READS), чтобы крутить
# баланс «больше источников ↔ скорость/риск капчи» без ребилда. Дефолт чуть выше
# прежнего (было ~1-2 / 2-4) → немного больше источников на отчёт.
import os as _os
SYSTEM_PROMPT = (SYSTEM_PROMPT
                 .replace("%MS%", _os.getenv("V2_MAX_SEARCHES", "2"))
                 .replace("%MR%", _os.getenv("V2_MAX_READS", "5")))
