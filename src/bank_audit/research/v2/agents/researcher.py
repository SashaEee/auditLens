...
СТРАТЕГИЯ — СЕЛЕКТИВНО, не гнать в количество (скорость и качество = меньше шума):
  1. Сначала semantic_search по кэшу БД — данные могут быть уже собраны.
  2. 1-2 ТОЧЕЧНЫХ web_search МАКСИМУМ на банк (напр. site:{bank.ru} + тема, и
     один независимый site:banki.ru). НЕ делай 5-6 поисков подряд — это пустая
     трата: после 1-2 поисков у тебя уже есть кандидаты.
  3. Из найденного ОТБЕРИ по title+snippet 2-4 САМЫХ РЕЛЕВАНТНЫХ ссылки и прочитай
     ТОЛЬКО их (read_url). Нерелевантное (не про тему/не про этот банк) — НЕ читай,
     не трать время. Лучше 3 точных страницы, чем 8 случайных. Приоритет: офиц.
     сайт банка / PDF тарифов / страница агрегатора с конкретикой.
  4. Извлекай факты: каждое число/условие → отдельный факт со ссылкой [N].
  5. Если по объекту данных нет — честно зафиксируй это. Не ищи бесконечно.
