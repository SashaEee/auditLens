...
SEARCH_REVIEWS_DB = ToolSpec(
    ...
    description=(
        ...
        "⚠ СРАВНЕНИЕ / ТОП банков по теме: передай banks=[\"Сбербанк\",\"ВТБ\","
        "\"Альфа-Банк\",\"Т-Банк\"] (+query=тема) — инструмент САМ сделает точечный "
        "bank-scoped поиск по КАЖДОМУ банку и вернёт by_bank:{банк:[жалобы]}. Это "
        "надёжнее: у каждого банка свой top-k, банки не вытесняют друг друга.\n"
        "query БЕЗ bank/banks — лишь общий рыночный top-k: НЕ покрывает все 217 банков "
        "и НЕ доказывает отсутствие жалоб. НИКОГДА не делай вывод «у банка X жалоб нет» "
        "по бесбанковому запросу.\n"
    ...
    parameters={
        ...
        "properties": {
            ...
            "banks": {"type": "array", "items": {"type": "string"}, "description":
                      "Список банков для СРАВНЕНИЯ/ТОПа — точечный поиск по каждому отдельно, "
                      "результат by_bank. Предпочтительно вместо bankless query при сравнении."},
