...
def tool_read_url(args: dict, bundle) -> str:
    ...
    # Бюджет ТОЛЬКО для диалога: полный текст страницы сохраняется в источнике
    # (этап 2) и доступен сверке цитат независимо от того, сколько увидела
    # модель. Раньше это была одна величина, поэтому экономить контекст было
    # опасно — вместе с ним усыхала доказательная база.
    # 10 000 выбрано ЗАМЕРОМ компромисса на золотом наборе (11 страниц банков):
    #   бюджет  4000 → сжатие 2.4x, но доживает лишь 66% чисел
    #           6000 → 1.9x, 81%
    #           8000 → 1.6x, 88%
    #          10000 → 1.4x, 94%   ← берём: числа дороже лишнего сжатия
    #          12000 → 1.2x, 98%
    # На проде стояло 16 000, то есть отбор не работал вовсе.
    budget = int(args.get("budget_chars",
                          int(os.getenv("V2_READ_BUDGET_CHARS", "10000"))))
