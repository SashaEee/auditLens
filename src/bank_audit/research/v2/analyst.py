...
ПРЕДМЕТ ВОПРОСА (проверь ПЕРВЫМ делом, до того как выбрать структуру).
Аудитор спросил о чём-то конкретном. Отчёт обязан отвечать ИМЕННО на это.
Разбери вопрос: о чём он — о СРАВНЕНИИ БАНКОВ, о ДОКУМЕНТЕ/ДАННЫХ РЕГУЛЯТОРА,
о ЖАЛОБАХ клиентов, о ПРОЦЕССЕ внутри Сбера?
  • Вопрос про документ регулятора («предельные значения ПСК за II квартал»,
    «указание ЦБ №…», «предписание ФАС») → ядро отчёта — САМ ДОКУМЕНТ: его
    реквизиты, дата публикации, период применения и ЗАПРОШЕННЫЕ значения
    целиком, по всем категориям. Банки — необязательный контекст в конце,
    и только если по ним есть факты в bundle.
  • Подмена предмета — грубейшая ошибка. Отчёт, где вместо таблицы ЦБ идёт
    сравнение ставок банков по витринам, аудитор считает НЕВЕРНЫМИ ДАННЫМИ,
    даже если каждое отдельное число в нём верное.
  • Если в bundle НЕТ того, о чём спросили, — так и напиши ПЕРВОЙ строкой после
    заголовка: «Прямого ответа на вопрос в собранных источниках нет: <чего
    именно не хватает>». Затем отдай то, что собрано, честно назвав это
    смежным контекстом. Развёрнутый ответ не на тот вопрос хуже короткого
    признания пробела: аудитор потратит время и не получит нужного.
...
а не пересказ таблицы дженерик-моделью):
  ...
  • Если отчёт сравнивает банки — ВСЕГДА выделяй позицию Сбера относительно
    рынка (дешевле/дороже/строже на столько-то ₽/п.п./раз), даже если вопрос
    прямо об этом не просит. Но если предмет вопроса — документ регулятора или
    внутренний процесс, НЕ притягивай сравнение с конкурентами ради этого
    правила: оно вытеснит ответ на заданный вопрос.
...
ЖЁСТКИЕ ПРАВИЛА:
  ...
  • НЕ СРАВНИВАЙ НЕСРАВНИМОЕ. Прежде чем написать «X выше Y», проверь, что это
    величины одной природы и одной базы. Типичные ловушки:
      – среднерыночное значение ПСК ≠ предельное значение (это разные колонки
        одной таблицы ЦБ, предельное больше среднерыночного на треть по формуле);
      – ставка по продукту конкретного банка ≠ ПСК категории (в ПСК входят
        страховки и комиссии, ставка — только проценты);
      – диапазон значений по рынку ≠ потолок, установленный регулятором;
      – ставка с господдержкой ≠ рыночная ставка;
      – годовая доходность ≠ доход за срок вклада.
    Если основания для сравнения нет — скажи об этом прямо, это и есть вывод
    аудитора. Ложное сравнение двух корректных чисел читается как ошибка в
    данных и обесценивает весь отчёт.
...
def profile_addendum(plan, bundle) -> str:
    """Тематические довески к системному промпту ПО ТИПУ вопроса.

    Раньше глоссарий автопереводов и прочие тематические ловушки были зашиты
    в статичный промпт и ехали в КАЖДЫЙ отчёт: вопрос про ипотеку получал
    наставления про автоплатежи. Теперь довесок собирается из плана.
    """
    hay = " ".join([
        getattr(bundle, "question", "") or "",
        getattr(plan, "product", "") or "",
        " ".join(getattr(plan, "product_synonyms", None) or []),
    ]).lower()
    parts: list[str] = []
    if any(k in hay for k in ("автоперевод", "автоплат", "автосписан", "подписк",
                              "рекуррент", "регулярный перевод")):
        parts.append(
            "# ГЛОССАРИЙ ТЕМЫ (НЕ путать термины)\n"
            ...
            "В жалобах «списали без ведома» — почти всегда автосписания по подпискам. "
            "Смешение терминов — грубая ошибка; добавь секцию «## Глоссарий» в отчёт.")
    if (getattr(plan, "question_nature", "") or "").strip().lower() == "regulatory":
        parts.append(
            "# РАМКА: ДОКУМЕНТ РЕГУЛЯТОРА\n"
            "Ядро отчёта — сам документ: точные реквизиты (номер, дата), период "
            "применения, запрошенные значения ЦЕЛИКОМ (все категории/строки из "
            "bundle). Секции «Сравнение условий» и «Рейтинг» уместны ТОЛЬКО если "
            "аудитор просил сопоставить банки с нормой. Позицию Сбера не "
            "притягивай, если вопрос о ней не спрашивает.")
...
async def write_report(client: AsyncOpenAI, bundle: KnowledgeBundle,
    ...
    if (not bundle.facts and not bundle.complaints and not bundle.insights
            and not bundle.regulations):
        # regulations в гейте обязательны: regulatory-агент пишет ТОЛЬКО нормы
        # и инсайты — отчёт «не удалось собрать данные» при собранной
        # нормативке был главным сценарием пустых ответов на вопросы о ЦБ.
