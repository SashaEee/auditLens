...
class BaseAgent:
    ...
    def _emit_status(self, **kw):
        ...
        ИНВАРИАНТ (важно): emit вызывается СИНХРОННО в event-loop потоке внутри
        run(). Дренаж очереди в оркестраторе (_run_missions_streaming) полагается
        на то, что каждый put_nowait происходит ДО завершения таски агента —
        иначе финальный слив очереди по empty() потеряет поздние события. НЕ
        переносить _emit в run_in_executor / fire-and-forget таску."""
        if not self._emit:
            return
        try:
    ...
    def _build_messages(self) -> list[dict]:
        ...
        user = ""
        # Исходный вопрос — ДОСЛОВНО. Агент видел только LLM-пересказ
        # кондуктора, и потерянный при пересказе квалификатор («за II квартал»,
        # «для ИП», «без учёта промо») был невосстановим: сборщики честно
        # собирали данные на смещённое задание.
        _q = (getattr(self.bundle, "question", "") or "").strip()
        if _q:
            user += (f"# ИСХОДНЫЙ ВОПРОС АУДИТОРА (дословно; задание ниже — "
                     f"его пересказ, при расхождении верь вопросу)\n{_q}\n\n")
        user += (
            f"# ЗАДАНИЕ\n{self.mission.goal}\n\n"
            f"# ОБЪЕКТЫ\n{', '.join(self.mission.subjects) or '(не заданы)'}\n"
        )
        # Сегмент клиента: розничная выдача глушит B2B-страницы по SEO, и
        # без явного наставления агент уносится в ипотеку/вклады (отчёт 199:
        # 20 из 21 источника — розница на вопрос про кредитование подрядчиков).
        if (getattr(self.bundle, "client_segment", "") or "") == "business":
            user += (
                "\n# СЕГМЕНТ: БИЗНЕС (ИП/юрлица)\n"
                "Вопрос про продукт ДЛЯ БИЗНЕСА. Розничные страницы (ипотека "
                "физлицам, вклады, потребкредиты, каталоги banki.ru) НЕ отвечают "
                "на вопрос — не читай их и не цитируй. Ищи: разделы «для "
                "бизнеса» на сайтах банков (/sme/, /s_m_business/, «малому "
                "бизнесу», «корпоративным клиентам»), пресс-релизы банков и "
                "отраслевые СМИ. Добавляй к запросам «для бизнеса», «для "
                "юридических лиц», «малый бизнес».\n")
        _syn = list(getattr(self.bundle, "product_synonyms", None) or [])
        if _syn:
            user += (
                f"\n# СИНОНИМЫ ПРОДУКТА\nБанки называют продукт по-разному: "
                f"{', '.join(_syn[:6])}. Если поиск по основному названию даёт "
                f"не тот сегмент или пусто — переформулируй запрос через эти "
                f"синонимы, а не повторяй тот же.\n")
