...
# Общий каталог кода парсеров. По умолчанию внутри пакета, но прод деплоится
# пересозданием контейнера — каталог в образе умирает вместе с ним, и парсеры
# после деплоя остаются без venv. На проде путь указывается через
# PARSER_CATALOG_DIR и монтируется томом.
CATALOG_DIR = Path(os.getenv("PARSER_CATALOG_DIR",
                             str(Path(__file__).resolve().parent / "catalog")))


# Каждая попытка — прогон парсера (до 900 с) + вызов модели + pip install.
# Двадцать попыток по умолчанию — это часы работы и сотни мегабайт на диске.
_MAX_FIX_ATTEMPTS = int(os.getenv("PARSER_MAX_FIX_ATTEMPTS", "3"))
