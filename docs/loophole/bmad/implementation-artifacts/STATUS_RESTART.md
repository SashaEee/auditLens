...
- [2026-09-07] Проблема: при скрытии контекста `sources` один ролевой сценарий с повторным назначением роли сохранил устаревшее ожидание видимости вкладки → Решение: проверять все ожидания контекстов в каждом сценарии до завершения TDD-цикла.
- [2026-09-07] Проблема: точечный cleanup-патч не совпал с фактическим текстом комментария в авторизации → Решение: брать контекст хунка непосредственно из текущего файла перед повторной правкой.
- [2026-09-07] Проблема: Playwright в песочнице не создаёт Windows named pipe (`WinError 5`) → Решение: повторять адресные browser-тесты с ограниченным внешним доступом, не меняя runtime-код.
- [2026-09-07] Проблема: CSV-runtime-тест переключается на вкладку источников после выгрузки, хотя это не видно из его первого действия → Решение: сохранять изолированную fixture `PARSER_CONTEXTS` для полного сценария, а не подменять её roleless-контекстом.
- [2026-09-07] Проблема: Ruff интерпретирует `.jsx` как Python и выдаёт ложные syntax errors → Решение: запускать Ruff только для Python-файлов, а JSX проверять адресными Playwright-сценариями.
- [2026-09-07] Проблема: фоновый `auditlens serve` завершился до открытия порта 8000 → Решение: повторять запуск с захватом stdout/stderr и проверять `/healthz` только после появления listener-а.
- [2026-09-07] Проблема: `auditlens serve` аварийно завершился с `ImportError: cannot import name 'main' from bank_audit.cli` → Решение: для локальной проверки запускать документированный `uvicorn bank_audit.web.app:app`; CLI-entrypoint исправлять отдельной задачей.
- [2026-09-07] Проблема: Uvicorn успешно выполнил startup, но получил `WinError 10048` при bind `127.0.0.1:8000` → Решение: перед повторным запуском проверять остаточные процессы и доступность порта, не менять приложение.
- [2026-09-07] Проблема: merge-патч fallback для JSX не совпал с форматированием актуального upstream-обработчика `/contexts` → Решение: брать дословный hunk из текущего upstream-варианта, не заменять callback целиком предположением.
- [2026-09-07] Проблема: статическая регрессия fallback не учла trailing comma в актуальном объекте `setAuthz` → Решение: сверять нормализованное утверждение с полным фактическим выражением после merge.
- [2026-09-07] Проблема: после merge в browser-тесте остался клавиатурный переход на скрытый `sources` → Решение: после разрешения конфликтов прогонять навигационные runtime-сценарии, а не только серверные контракты.
- [2026-09-07] Проблема: после правки runtime-навигации pytest выявил `IndentationError` в соседнем блоке → Решение: перед повторным browser-прогоном проверять синтаксис merge-файла и исправлять отступы по точному контексту.
- [2026-09-07] Проблема: после merge Ruff выявил неиспользуемый `capabilities_json` в browser-fixture → Решение: проверять lint уже после объединения upstream-fixtures и сохранять capability-данные только при фактической передаче в mock-ответ.
