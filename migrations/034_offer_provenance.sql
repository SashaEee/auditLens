...
-- Витрина отдаёт источник оффера: без него нельзя показать, откуда число.
-- CREATE OR REPLACE VIEW не разрешает вставлять колонку в середину («cannot
-- change name of view column») — новые поля дописываем строго В КОНЕЦ, порядок
-- существующих 24 колонок сохранён один в один (поймано 11.08.2026).
